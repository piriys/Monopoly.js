...
class UIHelpers {
  ...
  static rollDice(player) {
    ...
    const button = this.dialog(player, message, [
      new Option("accept", "Roll Dice")
    ]).get("accept");
  ...
  static payTax(player, tax) {
    ...
    const buttonText = `Pay \$${tax.amount}${
      settings.taxMultiplier !== 1 ? ` (x${settings.taxMultiplier})` : ""
    }`;
    const button = this.dialog(
      player,
      message,
      [new Option("accept", buttonText)],
      "💸"
    ).get("accept");
  ...
  static upgradeProperty(player, property) {
    ...
    const options = [];
    options.push(
      new Option("accept", `Upgrade for \$${property.currentUpgradeCost}`)
    );
    options.push(new Option("decline", "Cancel"));
  ...
  static payRent(player, property) {
    ...
    const buttonText = `Pay \$${property.calculatedRent}${
      settings.rentMultiplier !== 1 ? ` (x${settings.rentMultiplier})` : ""
...
class Player extends GameAsset {
  ...
  updateStatsDisplay() {
    ...
    this.playerStateDOM.innerText = `[${this.state.toUpperCase()}${
      this.state === "playing" || this.state === "bankrupt"
        ? ""
        : ` (${this.stateTurn})`
    }]`;
...
class RandomAction extends Space {
  ...
  resolve(player) {
    ...
    const button = UIHelpers.dialog(
      player,
      message,
      [new Option("accept", "Draw")],
      this.icon
    ).get("accept");
    button.addEventListener("click", () => {
      if (this.deck.length > 0) {
        ...
        switch (card.type.toLowerCase()) {
          case "movetoproperty":
            ...
            resolveFunction = () => {
              const propertySpaceIndex = game.spaces.findIndex(
                space => space.name === card.propertyName
              );
              player.moveTo(propertySpaceIndex);
              game.finishTurn();
            };
            break;
          case "withdraw":
        ...
        const resolveButton = UIHelpers.dialog(
          player,
          card.description,
          [new Option("accept", resolveButtonText)],
          card.icon
        ).get("accept");
...
class GoToJail extends Space {
  ...
  resolve(player) {
    ...
    const jailSpaceIndex = game.spaces.findIndex(
      space => space.type === "jail"
    );
    ...
    if (this.owner === undefined) {
      ...
      buttons.get("accept").addEventListener("click", () => {
        if (player.money >= this.price) {
          ...
          this.owner = player;
          console.log(
            `${this.name} is bought by ${player.name} for \$${this.price}`
          );
          this.updateDisplay();
        } else {
      ...
      console.log(
        `this property is owned by player. check if upgrade is available.`
      );
      //check if player own all property in color group
      const owned = game.spaces.reduce(
        (count, current) =>
          count +
          (current.owner === player && current.color === this.color ? 1 : 0),
        0
      );
      const total = game.spaces.reduce(
        (count, current) => count + (current.color === this.color ? 1 : 0),
        0
      );
      ...
      if (
        (this.color === "black" || owned === total) &&
        this.upgradeLevel < this.rent.length - 1
      ) {
        ...
        buttons.get("accept").addEventListener("click", () => {
          if (player.money >= this.currentUpgradeCost) {
            ...
            console.log(
              `${this.name} is upgraded by ${player.name} for \$${this.currentUpgradeCost}`
            );
            this.upgradeLevel++;
            this.updateDisplay();
          } else {
      ...
      console.log(
        `this property is owned by another player. transfer ${this.calculatedRent} to ${this.owner.name}`
      );
      const button = UIHelpers.payRent(player, this);
      button.addEventListener("click", () => {
        player.transfer(
          this.calculatedRent * settings.rentMultiplier,
          this.owner
        );
        game.finishTurn();
      });
...
class Game {
  ...
  play() {
    ...
    if (player.state !== "bankrupt") {
      if (player.state === "playing") {
        ...
        const message =
          player.stateTurn > 1
            ? `${player.stateTurn} Turn${
                player.stateTurn > 1 ? "s" : ""
              } Remaining in Jail`
            : "Last Turn in Jail";
        const buttonText = player.stateTurn > 1 ? "Skip Turn" : "Get Out";
        const button = UIHelpers.dialog(player, message, [
          new Option("accept", buttonText)
        ]).get("accept");
        button.addEventListener("click", () => {
          player.stateTurn--;
  ...
  rollDice(count = settings.dieCount) {
    ...
    button.addEventListener("click", () => {
      ...
      console.log(
        `${player.name} landed on ${position}/${this.spaces.length - 1}!`
      );
  ...
  nextTurn() {
    ...
    const activePlayerCount = game.players.reduce(
      (count, current) => count + (current.state !== "bankrupt" ? 1 : 0),
      0
    );
...
class GameHelpers {
  ...
  static resizeBoard() {
    ...
    board.transform.scale =
      Math.min(windowWidth / boardWidth, windowHeight / boardHeight) * 0.99;
    gameLoaderDOM.style["transform-origin"] = `${boardHeight /
      2}px ${boardWidth / 2}px`;
